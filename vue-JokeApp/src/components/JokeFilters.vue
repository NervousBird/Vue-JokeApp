...
const emit = defineEmits<{
    (event: 'updateFilters', info: any): void
}>()
...
watch(categoryString, (newInput, oldInput) => {
    ...
    if(oldInput.includes('Any') && categoryString.value.length > 1) {
        return categoryString.value = categoryString.value.filter((i: any) => i !== 'Any')
    }
...
watch(categoryFlag, (newInput, oldInput) => {    
    ...
    if(oldInput.includes('all') && newInput.includes('all')) {
        return categoryFlag.value = categoryFlag.value.filter((i: any) => i !== 'all')
    }
    createFilterString()
})
